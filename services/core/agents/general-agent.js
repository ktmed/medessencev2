 ...
 * Handles general radiology reports with exact text extraction
 */

const SpecializedAgent = require('./base-agent');

class GeneralAgent extends SpecializedAgent {
  constructor(multiLLMService = null) {
    super('general', multiLLMService);
  }
  ...
  async parseReport(reportText, language = 'de', metadata = {}) {
    console.log(`${this.name}: Processing general report`);
    ...
    try {
      if (this.llmService) {
        ...
        // If LLM processing succeeded, enhance with general-specific data
        if (baseResult.metadata?.aiGenerated) {
          return this.enhanceWithGeneralData(baseResult, reportText, metadata);
        }
      }
    } catch (error) {
  ...
  /**
   * Enhance LLM-generated result with general-specific data
   */
  enhanceWithGeneralData(baseResult, reportText, metadata) {
    console.log(`${this.name}: Enhancing LLM result with general-specific data`);
    
    // Add general-specific sections to the base result
    const exactSections = this.exactExtractor.extractSections(reportText);
