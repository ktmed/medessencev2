/**
 * Cardiac Report Agent
 * Handles cardiac reports with exact text extraction
 */

const SpecializedAgent = require('./base-agent');

class CardiacAgent extends SpecializedAgent {
  constructor(multiLLMService = null) {
    super('cardiac', multiLLMService);
  }
  ...
  async parseReport(reportText, language = 'de', metadata = {}) {
    console.log(`${this.name}: Processing cardiac report`);
    ...
    try {
      if (this.llmService) {
        ...
        // If LLM processing succeeded, enhance with cardiac-specific data
        if (baseResult.metadata?.aiGenerated) {
          return this.enhanceWithCardiacData(baseResult, reportText, metadata);
        }
      }
    } catch (error) {
  ...
  /**
   * Enhance LLM-generated result with cardiac-specific data
   */
  enhanceWithCardiacData(baseResult, reportText, metadata) {
    console.log(`${this.name}: Enhancing LLM result with cardiac-specific data`);
    
    // Add cardiac-specific sections to the base result
    const exactSections = this.exactExtractor.extractSections(reportText);
