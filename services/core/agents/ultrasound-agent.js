/**
 * Ultrasound Report Agent
 * Handles ultrasound reports with exact text extraction
 */

const SpecializedAgent = require('./base-agent');

class UltrasoundAgent extends SpecializedAgent {
  constructor(multiLLMService = null) {
    super('ultrasound', multiLLMService);
  }
  ...
  async parseReport(reportText, language = 'de', metadata = {}) {
    console.log(`${this.name}: Processing ultrasound report`);
    
    // First try the base agent's LLM-based parsing
    try {
      console.log(`${this.name}: LLM service available:`, !!this.llmService);
      if (this.llmService) {
        console.log(`${this.name}: Attempting LLM-based parsing`);
        const baseResult = await super.parseReport(reportText, language, metadata);
        console.log(`${this.name}: LLM result metadata:`, baseResult.metadata);
        
        // If LLM processing succeeded, enhance with ultrasound-specific data
        if (baseResult.metadata?.aiGenerated) {
          console.log(`${this.name}: LLM processing succeeded, enhancing result`);
          return this.enhanceWithUltrasoundData(baseResult, reportText, metadata);
        } else {
          console.log(`${this.name}: LLM processing did not generate AI result`);
        }
      } else {
        console.log(`${this.name}: No LLM service available`);
      }
    } catch (error) {
  ...
  /**
   * Enhance LLM-generated result with ultrasound-specific data
   */
  enhanceWithUltrasoundData(baseResult, reportText, metadata) {
    console.log(`${this.name}: Enhancing LLM result with ultrasound-specific data`);
    
    // Add ultrasound-specific sections to the base result
    const exactSections = this.exactExtractor.extractSections(reportText);
...
module.exports = UltrasoundAgent;