...
from app.services.exact_text_extractor import exact_text_extractor
...
class OpenAIProvider(BaseAIProvider):
    ...
    def _parse_report_response(self, content: str) -> Dict[str, Any]:
        # Extract exact sections from the generated content
        sections = exact_text_extractor.extract_sections(content)
        
        # Map extracted sections to report structure
        result = {
            "BEFUND": "",
            "BEURTEILUNG": "",
            "EMPFEHLUNGEN": "",
        ...
        for section in sections:
            if section.name.lower() in ['befund', 'befunde', 'findings']:
                result["BEFUND"] = section.content
            elif section.name.lower() in ['beurteilung', 'zusammenfassung', 'assessment']:
                result["BEURTEILUNG"] = section.content
            elif section.name.lower() in ['empfehlung', 'procedere', 'recommendations']:
                result["EMPFEHLUNGEN"] = section.content
            elif section.name.lower() in ['technik', 'technische parameter']:
                ...
                for line in lines:
                    if ':' in line:
                        ...
                        tech_params[key.strip()] = value.strip()
                result["TECHNISCHE_PARAMETER"] = tech_params
